...
```{r}
US_wiki_url <- read_html("https://en.wikipedia.org/wiki/List_of_U.S._states_and_territories_by_population")
```

1.  Make sure the name of columns and the first row is correct

    ```{r}
    US_wiki_tables <- US_wiki_url |>
      html_table(fill = TRUE)
    US_pop_rank <- US_wiki_tables[[1]]
    ```

    ```{r}

    US_pop_rank_clean <- US_pop_rank[-1, ]
