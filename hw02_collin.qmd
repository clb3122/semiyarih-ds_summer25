...
```{r}
find_z <- function(x, mean, sd) { 
  z_score <- (x - mean) / sd
  return(z_score)
}
find_z(25.77, 23.54, 2.442)
```
...
```{r}
Q2_log <- function(x, prime_num) {
  nat_log <- log(x)
  com_log <- log10(x)
  cube_rt <- prime_num^(1/3)
  unrounded <- (nat_log * com_log) / cube_rt
...
names(sd_mtcars) <- names(mtcars)
column_names <- names(mtcars)
for (i in seq_along(column_names)) {
  sd_mtcars[i] <- sd(mtcars[[column_names[i]]])
  return(sd_mtcars)
}
sd_mtcars
```
