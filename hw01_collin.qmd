...
-   **Problem 2**: Is there a relationship between the age of a plane and its delays?
    ...
    Plane age is available in the planes data set, so I need to join planes and flights:
    ...
    Plot relationship between make year and departure delay:

    ```{r}
    library(ggplot2)
    ggplot(flights_joined, aes(x = year.y, y = dep_delay)) + geom_col() #+ scale_y_discrete(breaks = (0:100))
    ```

    There appear to be more delays among newer planes rather than older ones
...
-   **Problem 3**: Find the 10 days of the year that have the highest median departure delay, then select all flights from those 10 days.
    ...
    Create a df for the top ten delay days:

    ```{r}
    top10 <- flights %>%
      group_by(month, day) %>%
      summarize(med_delay = median(dep_delay, na.rm = TRUE)) %>%
    ...
    All flights from top 10 delay days:

    ```{r}
    top10_all <- flights %>% 
      inner_join(top10, by = c("month", "day"))
    top10_all
    ```
